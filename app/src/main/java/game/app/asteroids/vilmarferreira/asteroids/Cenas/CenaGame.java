...
public class CenaGame extends AGScene{
    ...
    private AGSprite meteoroGrande=null;
    ...
    public void init() {
        ...
        meteoroGrande= createSprite(R.mipmap.meteoro_mod_dois,1,1);
        meteoroGrande.setScreenPercent(5,10);
    ...
    public void loop()
    {
        ...
        if (nave.vrPosition.getX() < (AGScreenManager.iScreenWidth - nave.getSpriteWidth() / 2) && AGInputManager.vrAccelerometer.getAccelX() > 2) {
            nave.vrPosition.setX(nave.vrPosition.getX() + 15);
        ...
        //verifica se o tempo acabou para criar novos meteoros
        if(time.isTimeEnded())
        {
            createMeteoro();
            time.restart();
        }
        //verifica se o tempo da fase acabou para mudar a fase
    ...
    public void createMeteoro ()
    {
        ...
        AGSprite meteoro;
        meteoro = createSprite(R.mipmap.meteoro_mod_dois, 1, 1);
        random.setSeed(System.currentTimeMillis());
